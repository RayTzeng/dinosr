...
@register_model("semi-dinosr", dataclass=SemiDinosrAudioConfig)
class SemiDinosrModel(BaseFairseqModel):
    ...
    def apply_mask(
        ...
        if self.mask_prob > 0:
            if mask_indices is None:
                ...
                if nonmask_indices is not None:
                    mask_indices = mask_indices & ~nonmask_indices
            try:
    ...
    def forward(
        ...
        ###### Check if there is any labeled samples ######
        if target_list is not None:
        ...
        if mask:
            ...
            x, mask_indices = self.apply_mask(
                ...
                nonmask_indices=labeled_indices,
            )
            #######################
        else:
            x = features
        ...
        with torch.no_grad():
            ...
            if self.discrete:
                if labeled_indices is not None:
                    l_target_layer_results = [
                        tl[labeled_indices] for tl in target_layer_results
                    ]
                try:
                    target_layer_results = [
        ...
        if self.discrete:
            ...
            for i,target in enumerate(target_layer_results):
                ...
                # Compute loss on labeled part
                if labeled_indices is not None:
                    l_teacher_repr = l_target_layer_results[i]
                    for l_target in l_target_list:
                        l_pred = self.heads[i](l_x).float()
                        l_pred = F.log_softmax(l_pred,dim=-1)
                        onehot_l_target = F.one_hot(l_target.long(), num_classes=self.heads[i].out_features).float()
                        smoothed_l_target = label_smoothing(onehot_l_target)
                        l_loss = torch.sum(-smoothed_l_target*l_pred,dim=-1)
                        l_losses = l_losses + l_loss
                    
                    # IPython.embed()
                    onehot_target = torch.cat((onehot_target, onehot_l_target), 0)
                    target = torch.cat((target, l_teacher_repr), 0)
                    pred = torch.cat((pred, l_pred), 0)
            ...
            result["losses"]["m_cross_entropy"] = (m_losses/self.n_codebooks).sum()
            result["losses"]["l_cross_entropy"] = (l_losses/self.n_codebooks).sum()
